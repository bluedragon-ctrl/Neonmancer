 ...
 *
 * Solid are static block cells, object cells and everything outside the
...
export class Grid {
   ...
   * @param {{ at: number[] }[]} room.objects typed objects (one cell each)
   * @param {number[][]} room.holes hole tiles [x, z]
   */
  constructor({ size, cells, objects, holes }) {
    this.size = size;
    this.cells = new Set(cells.map(([x, y, z]) => key(x, y, z)));
    for (const { at } of objects) this.cells.add(key(...at));
